...
def inpaint(args):
    ...
    saved_G = torch.load(args.gan_path + args.g_path + ".pth")
    saved_D = torch.load(args.gan_path + args.d_path + ".pth")
    ...
    for i, (corrupted_images, original_images, masks, weighted_masks) in enumerate(dataloader):
        ...
        for epoch in range(args.optim_steps):
            ...
            prior_loss = torch.mean(netD(gen_images)) * args.prior_weight
            inpaint_loss = context_loss + prior_loss

            inpaint_loss.backward()
            inpaint_opt.step()
            if epoch % 500 == 0:
                print("Epoch: {}/{}\tLoss: {:.3f}\tContext loss: {:.3f}\tPrior loss: {:.3f}\r".format(1+epoch, args.optim_steps, context_loss, prior_loss, inpaint_loss))
        print("")
